...
public class GameSparker extends Applet implements Runnable {
	...
	public static final String carModels[] = {
			"skyliner34", "venom", "wander", "stormrider", "vigero", "silva", //starter
			"mazdarx7", "razorback", "supra", "semitruck", //boss
			"amethyst", "digger", "apollo", //big 3
			"drmonster" //final
	};
	...
	public static final String extraModels[] = {
	        ...
	        "offroadtakeoff", "offlanding", "sdroad", "twister1d", "twister2d", "dustyend", "dustramp1", "dustramp2", "dramp2",
	        "bramp", "broad", "quarterpipe",
	        
	        "snowstartroad", "snowroad", "snowroadend", "snowturn", "snowbump", "snowramp", 
	        "snowbigramp", "snowroadlanding", "snowroadtakeoff", "snowcheckpoint", "snowblend", "snowcheckpoint2"
	   };
	...
	public static final String stageDir = "data/stages/";
	...
	private void cropit(final Graphics2D graphics2d, final int i, final int i_98_) {
		...
		if (i != 0) {
			if (i < 0) {
				graphics2d.fillRect(apx + i, apy - (int) (25.0F), Math.abs(i), (int) (500.0F));
			} else {
				graphics2d.fillRect(apx + (int) (900.0F), apy - (int) (25.0F), i, (int) (500.0F));
			}
		}
		if (i_98_ != 0) {
			if (i_98_ < 0) {
				graphics2d.fillRect(apx - (int) (25.0F), apy + i_98_, (int) (950.0F), Math.abs(i_98_));
			} else {
				graphics2d.fillRect(apx - (int) (25.0F), apy + (int) (550.0F), (int) (950.0F), i_98_);
			}
		}
	...
	public void paint(Graphics g) {
		...
		apx = (int) (getWidth() / 2 - 450.0F);
		apy = (int) (getHeight() / 2 - 250.0F);
		...
		try (BufferedReader bufferedreader = new BufferedReader(new FileReader(new File(stageDir + checkpoints.stage + ".txt")))) {
			for (String line; (line = bufferedreader.readLine()) != null;) {
				...
				if (line.startsWith("set")) {
					int k1 = Utility.getint("set", line, 0) -2;
					k1 += 6;
				...
				if (line.startsWith("chk")) {
					int l1 = Utility.getint("chk", line, 0) -2;
					l1 += 6;
				...
				if (line.startsWith("fltchk")) {
					int l1 = Utility.getint("fltchk", line, 0) -2;
					l1 += 6;
				...
				if (line.startsWith("fix")) {
					int i2 = Utility.getint("fix", line, 0) -2;
					i2 += 6;
	...
	public void run() {		
		...
		if (l >= 1 && l <= 25) {		
			 ...
			 * Note: that is an L
			 */
			xtgraphics.unlocked = 24;
			if (xtgraphics.unlocked != 25)
		...
		do {			
			...
			if (xtgraphics.fase == -5) {
				...
				if (flag) {
					if (checkpoints.stage == xtgraphics.unlocked && xtgraphics.winner && xtgraphics.unlocked != 25)
				...
				xtgraphics.ctachm(xm, ym, mouses, u[0]);
				if (checkpoints.stage == 25 && xtgraphics.winner)
					catchlink(1);
				if (mouses == 2)
					mouses = 0;
				if (mouses == 1)
			...
			if(xtgraphics.fase == 58)
                                       {
                xtgraphics.carspergame(checkpoints.stage);
            }
			if (xtgraphics.fase == 2) {
			...
			if (xtgraphics.fase == 1) {
				...
				int i3 = 0;
				int ai[] = new int[300];
				for (int k5 = xtgraphics.nplayers; k5 < notb; k5++)
			...
			if (xtgraphics.fase == 0) {
				...
				if (xtgraphics.starcnt < 38) {
					if (view == 0) {
						...
						//xtgraphics.stat(amadness, checkpoints, u[0], acontol, acontol[], true);
	...
	public void catchlink(int i) {
		if (!lostfcs) {
			if (i == 0)
				if (xm > 0 && xm < 900 && ym > 110 && ym < 169 || xm > 210 && xm < 460 && ym > 240 && ym < 259) {
